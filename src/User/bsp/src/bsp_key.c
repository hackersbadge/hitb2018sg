...
*		V1.2    2015-08-08   K1，K2，K3独立按键进行排他判断，修改  IsKeyDown1()等函数
*
...
/* 按键口对应的RCC时钟 */
#define RCC_ALL_KEY 	(RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOB | RCC_APB2Periph_AFIO)

#define GPIO_PORT_K1    GPIOB
#define GPIO_PIN_K1	    GPIO_Pin_14
...
static void bsp_InitKeyVar(void)
{
	...
	s_tBtn[KID_KEY_U].LongTime = 100;
	s_tBtn[KID_KEY_U].RepeatSpeed = 5;	/* 每隔50ms自动发送键值 */

	s_tBtn[KID_KEY_D].LongTime = 100;
	s_tBtn[KID_KEY_D].RepeatSpeed = 5;	/* 每隔50ms自动发送键值 */

	s_tBtn[KID_KEY_L].LongTime = 100;
	s_tBtn[KID_KEY_L].RepeatSpeed = 5;	/* 每隔50ms自动发送键值 */

	s_tBtn[KID_KEY_R].LongTime = 100;
	s_tBtn[KID_KEY_R].RepeatSpeed = 5;	/* 每隔50ms自动发送键值 */
